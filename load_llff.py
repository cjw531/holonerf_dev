...
def _load_data(basedir, factor=None, width=None, height=None, load_imgs=True):
    ...
    return poses, bds, imgs, imgfiles # return list of img file names as well
...
def render_path_spiral(c2w, up, rads, focal, zdelta, zrate, rots, N):
    ...
    for theta in np.linspace(0., np.pi * 2, N+1)[:-1]: # frame increased (avoid lagging) by multiplying 2 to N
...
def center_n_rotate_poses(poses, bds):
    ...
    offset_z = 1.5 # for spideramn
    # offset_z = 1.2 # for polaroid
    ...
    scale = 2 # for spiderman
    # scale = 0.5 # for polaroid
    radcircle *= scale
    new_poses = []
