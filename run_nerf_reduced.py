...
def render_rays(ray_batch,
    ...
    def raw2outputs(raw, z_vals, rays_d):
        ...
        rgb_map = tf.reduce_sum(input_tensor=weights[..., None] * rgb, axis=-2)  # [N_rays, 3]
...
def config_parser():
    ...
    # reduced training for performance testing
...
def train():
    ...
    if os.path.isdir(testimgdir) == False:
        os.system("mkdir -p " + testimgdir)
    ...
    if args.dataset_type == 'llff':
        images, poses, bds, render_poses, i_test, imgfiles = load_llff_data(args.datadir, args.factor,
                                                                  recenter=True, bd_factor=.75,
                                                                  spherify=args.spherify, 
                                                                  center_n_rotate=args.center_n_rotate)
        hwf = poses[0, :3, -1]
        poses = poses[:, :3, :4]
        ...
        if os.path.isfile(args.basedir + '/' + args.expname + '/filename_set.csv') == False:
            csv_data = open(args.basedir + '/' + args.expname + '/filename_set.csv', 'w')
            ...
            for i in np.arange(int(images.shape[0])):
                set_type = ''
                if i in i_train: set_type = 'train'
                elif i in i_test: set_type = 'test'
                elif i in i_val: set_type = 'validation'
                elif i in i_hidden: set_type = 'hidden'
                else: set_type = 'null'

                with open(args.basedir + '/' + args.expname + '/filename_set.csv', 'a') as f:
                    writer = csv.writer(f) # create the csv write
                    writer.writerow([i, imgfiles[i], set_type]) # write a row to the csv file
    ...
    elif args.render_hidden: render_poses = np.array(poses[i_hidden])
    ...
    if args.render_only:
        ...
        if args.render_test: # render_test switches to test poses
            ...
            savedir_prefix = 'test'
        elif args.render_hidden: # render_hidden
            images = images[i_hidden]
            savedir_prefix = 'hidden'
        else: # Default is smoother render_poses path
            images = None
            savedir_prefix = 'path'

        testsavedir = os.path.join(basedir, expname, 'renderonly_{}_{:06d}'.format(savedir_prefix, start))
    ...
    # create runtime logger csv file if it is not render_only = True
    csv_data = open(args.basedir + '/' + args.expname + '/psnr_runtime.csv', 'w')
    csv_writer = csv.writer(csv_data)
    csv_writer.writerow(['iter', 'psnr', 'loss', 'runtime (sec)'])
    csv_data.close()

    start_time = time.time() # runtime logger start
    ...
    if args.reduced_train: print('HIDDEN views are', i_hidden)

    # Summary writers
    writer = tf.compat.v2.summary.create_file_writer(logdir=os.path.join(basedir, 'summaries', expname))
    ...
    for i in range(start, N_iters):
        ...
        if i % args.i_print == 0 or i < 10:
            ...
            with open(args.basedir + '/' + args.expname + '/psnr_runtime.csv', 'a') as f:
                writer = csv.writer(f) # create the csv write
                writer.writerow([i, psnr.numpy(), loss.numpy(), round(time.time() - start_time, 2)]) # write a row to the csv file

            with summary_ops_v2.record_summaries_every_n_global_steps(args.i_print):
            ...
            if i % args.i_img == 0:
                ...
                if i == 0: os.makedirs(testimgdir, exist_ok=True)
                imageio.imwrite(os.path.join(testimgdir, '{:06d}.png'.format(i)), to8b(rgb))

                with summary_ops_v2.record_summaries_every_n_global_steps(args.i_img):
